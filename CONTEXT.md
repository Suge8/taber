...
Taber 是一个强大、全能的浏览器侧边栏 Agent。它阅读网页、提取文档与图片、跨标签导航、操作页面、生成文件，并把可复用的站点经验沉淀为技能。

## 语言

**浏览器 Agent**:
一个强大的侧边栏助手，读取并操作浏览器页面，工具过程有迹可循。
...
**站点技能**:
浏览器 Agent 按站点沉淀的可复用先验知识（流程、选择器、陷阱、恢复方法），以 `/skills/*.md` frontmatter 文件形态暴露给模型，由 Agent 通过 fs 工具在任务中写入，用户在侧边栏管理；内置种子技能覆盖大站的 fetch-first 接口。它是先验而非事实，与页面实时状态冲突时以页面为准。
_Avoid_: 手写操作手册、把技能当权威指令、存储密钥或个人数据、独立于 fs 的第二套技能入口

**文件工作区**:
每个会话的 `/workspace` 文件空间，存用户上传（PDF/Word/文本）和 Agent 产出（Markdown/HTML/CSV/Word）；通过 fs 工具读写，Markdown 可转 Word，用户从侧边栏下载或打印导出 PDF；随会话清理。
_Avoid_: 让模型手写二进制、伪 PDF 生成、脱离本地 Agent 数据库的第二套文件存储
...
**文档提取**:
将网页、选区、远程 URL（网页/PDF）和工作区文件转换为 Agent 可读的 Markdown、纯文本或结构化内容的能力；公开静态内容 fetch-first，不开 tab 直接拓取，需登录态或 JS 渲染才走受控标签页。
_Avoid_: 仅 DOM 文本、重型通用解析平台、一律开 tab 读公开页面
