...
Taber 在用户发消息启动任务时锁定工作区：background 使用侧边栏所属窗口当时可操作的 active http/https tab，写入 `runningTask.targetTabId` 与 `task.started.payload.context`。打开侧边栏不锁定 tab，避免用户只是查看侧边栏就改变后续任务起点。
...
- `navigate.switchTab`：先确认旧 target 仍存在且可操作，再切换到目标可操作 tab。
...
失效处理：target tab 被关闭、变为 `chrome://`/`edge://` 等不可操作 URL，或 target 权限不足时，任务进入 fatal error 并写入 `task.failed`；Taber 不回退到其他 active tab，因为那会让 Agent 在用户没确认的页面继续操作。
